...
public class NewDealPage {
	...
	private void setPreference(WebDriver driver, ExtentTest step) throws Exception {
		try {
			...
			if (!preferenceSetOnce) {
				...
				List<WebElement> preferenceElements = driver
						.findElement(By.id(newDealLocator.getString("list-PreferenceList")))
						.findElements(By.xpath(newDealLocator.getString("label-PreferenceName")));
				...
				for (int index = 0; index < preferences.length; index++) {
					if (preferenceList.contains(preferences[index])) {
						preferenceElements.get(preferenceList.indexOf(preferences[index]))
								.findElement(By.id(newDealLocator.getString("checkbox-Preference"))).click();
					}
				}
		...
		while (contactIterator.hasNext()) {
			...
			try {
				...
				List<WebElement> searchResult = driver
						.findElements(By.id(newDealLocator.getString("input-ContactSearchResult")));
	...
	private void setDealContacts(WebDriver driver, ExtentTest step, String dealName, JSONArray contactList)
		...
		while (contactIterator.hasNext()) {
			String contact = contactIterator.next().toString();
			try {
				utility.addToExpectedDetails(expectedDealDetails, dealName.toLowerCase().trim(),
				...
				Thread.sleep(500);
				List<WebElement> searchResult = driver
						.findElements(By.id(newDealLocator.getString("input-DealContactSearchResult")));
				Iterator<WebElement> searchResultIterator = searchResult.iterator();
				while (searchResultIterator.hasNext()) {
					WebElement result = searchResultIterator.next();
					if (!result.isSelected()) {
	...
	protected void addAccounts(WebDriver driver, ExtentTest step, JSONArray accounts) throws Exception {
		...
		while (accountIterator.hasNext()) {
			...
			try {
				expectedAccountDetails.put(account.getString("account_search_input").toLowerCase().trim(),
						new ArrayList<String>());
				Select countrySelect = new Select(
						driver.findElements(By.id(newDealLocator.getString("select-AccountCountry"))).get(1));
				countrySelect.selectByVisibleText(account.getString("account_country"));

				Select currencySelect = new Select(
						driver.findElement(By.xpath(newDealLocator.getString("select-AccountCurrency"))));
				currencySelect.selectByVisibleText(account.getString("account_currency"));

				Select accountIdentifierSelect = new Select(
						driver.findElement(By.id(newDealLocator.getString("select-AccountIdentifierKey")))
								.findElement(By.tagName("select")));
				accountIdentifierSelect.selectByVisibleText(account.getString("account_identifier_key"));

				Select searchParameterSelect = new Select(
						driver.findElement(By.id(newDealLocator.getString("select-AccountSearchParameter"))));
				searchParameterSelect.selectByVisibleText(account.getString("account_search_parameter"));

				driver.findElement(By.id(newDealLocator.getString("input-AccountSearchInput")))
						.sendKeys(account.getString("account_search_input"));
				driver.findElement(By.id(newDealLocator.getString("button-AccountSearch"))).click();

				// Select accountOBOFlag = new
				// Select(driver.findElement(By.xpath(newDealLocator.getString("select-AccountOBOFlag"))));
				// accountOBOFlag.selectByVisibleText(account.getString("account_OBO_Flag"));
				if (!utility.isElementHiddenNow(driver,
						By.xpath(newDealLocator.getString("label-AccountAlreadyOnboarded")))) {
					actualResponseMessages
							.add(driver.findElement(By.xpath(newDealLocator.getString("label-AccountAlreadyOnboarded")))
									.getText());
					step.log(Status.INFO,
							"Account: " + account.getString("account_search_input") + " already added to deal");
				...
				if (!account.getBoolean("account_balance_check")) {
					WebElement accountBalanceCheck = driver
							.findElement(By.id(newDealLocator.getString("checkbox-AccountBalanceCheck")));
				...
				if (!utility.isElementHiddenNow(driver, By.id(newDealLocator.getString("label-Title")))) {
					actualResponseMessages.add(
							utility.extractTitleMessage(driver, step, By.id(newDealLocator.getString("label-Title"))));
				...
				if (utility
						.extractColumnsFromTable(driver, step,
								By.xpath(newDealLocator.getString("table-AccountNumber")), 0)
						.contains(account.getString("account_search_input"))) {
	...
	protected void addPartyContacts(WebDriver driver, ExtentTest step, String partyName, JSONArray partyContacts)
			throws Exception {
		...
		while (partyContactIterator.hasNext()) {
			...
			try {
				utility.addToExpectedDetails(expectedPartyDetails, partyName.toLowerCase().trim(),
						"contact:" + partyContact.getString("party_contact_name").toLowerCase().trim());

				driver.findElement(By.id(newDealLocator.getString("button-NewPartyContact"))).click();

				if (partyContact.has("party_link_contact")) {

					List<WebElement> partyContactTables = driver
							.findElement(By.id(newDealLocator.getString("grid-PartyContactList")))
							.findElements(By.id(newDealLocator.getString("table-PartyTable")));
					List<String> partyContactNameList = utility.extractColumnsFromTable(driver, step,
							partyContactTables, 1);
					int partyContactIndex = partyContactNameList.indexOf(partyContact.getString("party_contact_name"));

					driver.findElement(By.id(newDealLocator.getString("grid-PartyContactList")))
							.findElements(By.id(newDealLocator.getString("checkbox-PartyCheckbox")))
							.get(partyContactIndex).click();
					driver.findElement(By.id(newDealLocator.getString("button-LinkPartyContact"))).click();
					utility.waitForProgressBarToLoad(driver);

					if (!utility.isElementHiddenNow(driver,
							By.xpath(newDealLocator.getString("button-SubmitDealOK")))) {
						step.log(Status.FAIL,
								"Error while adding Party Contact: " + partyContact.getString("party_contact_name"));
						report.addStepFailScreenshot(driver, step);
						throw new Exception(
								"Error while adding Party Contact: " + partyContact.getString("party_contact_name"));
					}
				...
				driver.findElement(By.id(newDealLocator.getString("input-PartyContactName")))
						.sendKeys(partyContact.getString("party_contact_name"));
				...
				if (partyContact.getBoolean("party_contact_subinstruction_notification")) {
					driver.findElement(By.id(newDealLocator.getString("checkbox-PartySubInstructionNotification")))
							.click();
				}

				driver.findElement(By.id(newDealLocator.getString("input-PartyContactEmail")))
						.sendKeys(partyContact.getString("party_contact_email"));

				driver.findElement(By.id(newDealLocator.getString("input-PartyContactDesignation")))
						.sendKeys(partyContact.getString("party_contact_designation"));

				driver.findElement(By.id(newDealLocator.getString("input-PartyContactWorkPhone")))
						.sendKeys(partyContact.getString("party_contact_work_phone"));

				driver.findElement(By.id(newDealLocator.getString("input-PartyContactMobilePhone")))
						.sendKeys(partyContact.getString("party_contact_mobile_phone"));

				driver.findElement(By.id(newDealLocator.getString("input-PartyContactStreet")))
						.sendKeys(partyContact.getString("party_contact_street"));

				driver.findElement(By.id(newDealLocator.getString("input-PartyContactTown")))
						.sendKeys(partyContact.getString("party_contact_town"));

				driver.findElement(By.id(newDealLocator.getString("input-PartyContactPin")))
						.sendKeys(partyContact.getString("party_contact_pin"));

				driver.findElement(By.id(newDealLocator.getString("input-PartyContactState")))
						.sendKeys(partyContact.getString("party_contact_state"));

				driver.findElement(By.id(newDealLocator.getString("input-PartyContactCountry")))
						.sendKeys(partyContact.getString("party_contact_country"));
	...
	protected void addPartyAccounts(WebDriver driver, ExtentTest step, String partyName, JSONArray partyAccounts)
			throws Exception {

		Iterator<Object> partyAccountIterator = partyAccounts.iterator();
		driver.findElement(By.id(newDealLocator.getString("tab-PartyTabSet"))).findElements(By.tagName("li")).get(2)
				.click();
		...
		while (partyAccountIterator.hasNext()) {
			...
			try {
				utility.addToExpectedDetails(expectedPartyDetails, partyName.toLowerCase().trim(),
						"account:" + partyAccount.getString("party_account_beneficiary_account").toLowerCase().trim());

				driver.findElement(By.id(newDealLocator.getString("button-NewPartyAccount"))).click();

				if (partyAccount.has("party_link_account")) {

					List<WebElement> partyAccountTables = driver
							.findElement(By.id(newDealLocator.getString("grid-PartyAccountList")))
							.findElements(By.id(newDealLocator.getString("table-PartyTable")));
					List<String> partyAccountList = utility.extractColumnsFromTable(driver, step, partyAccountTables,
							2);
					Integer partyAccountIndex = null;

					for (int index = 0; index < partyAccountList.size(); index++) {
						if (partyAccountList.get(index)
								.contains(partyAccount.getString("party_account_beneficiary_account"))) {
							partyAccountIndex = index;
						}
					}

					driver.findElement(By.id(newDealLocator.getString("grid-PartyAccountList")))
							.findElements(By.id(newDealLocator.getString("checkbox-PartyCheckbox")))
							.get(partyAccountIndex).click();
					driver.findElement(By.id(newDealLocator.getString("button-LinkPartyAccount"))).click();
					utility.waitForProgressBarToLoad(driver);

					if (!utility.isElementHiddenNow(driver,
							By.xpath(newDealLocator.getString("button-SubmitDealOK")))) {
						step.log(Status.FAIL, "Error while adding Party Account: "
								+ partyAccount.getString("party_account_beneficiary_account"));
						report.addStepFailScreenshot(driver, step);
						throw new Exception("Error while adding Party Account: "
								+ partyAccount.getString("party_account_beneficiary_account"));
					}
					continue;
				}
				...
				Thread.sleep(1000);
				driver.findElement(By.id(newDealLocator.getString("select-PartyAccountPaymentSystem")))
						.findElement(By.tagName("input")).click();
				utility.selectFromDivMenu(driver, step,
						driver.findElement(By.id(newDealLocator.getString("select-PartyAccountPaymentSystem")))
								.findElement(By.tagName("ul")),
						partyAccount.getString("party_account_payment_system"));
...
//				driver.findElement(By.id(newDealLocator.getString("input-PartyAccountBeneficiaryBIC"))).sendKeys(partyAccount.getString("party_account_beneficiary_BIC"));
				...
				driver.findElement(By.id(newDealLocator.getString("input-PartyAccountBeneficiaryName")))
						.sendKeys(partyAccount.getString("party_account_beneficiary_name"));

				driver.findElement(By.id(newDealLocator.getString("input-PartyAccountBeneficiaryAddress")))
						.sendKeys(partyAccount.getString("party_account_beneficiary_address"));
...
//				partyAccountIBAN.selectByVisibleText(partyAccount.getString("party_account_IBAN"));
				...
				Select partyAccountBeneficiaryCountrySelect = new Select(
						driver.findElement(By.id(newDealLocator.getString("select-PartyAccountBeneficiaryCountry"))));
				partyAccountBeneficiaryCountrySelect
						.selectByVisibleText(partyAccount.getString("party_account_beneficiary_country"));
...
//				driver.findElement(By.id(newDealLocator.getString("input-PartyAccountBeneficiaryCurrency"))).sendKeys(partyAccount.getString("party_account_beneficiary_currency"));
				...
				driver.findElement(By.id(newDealLocator.getString("input-PartyAccountDestinationAccount")))
						.sendKeys(partyAccount.getString("party_account_beneficiary_account"));

				driver.findElement(By.id(newDealLocator.getString("input-PartyAccountDescription")))
						.sendKeys(partyAccount.getString("party_account_description"));
	...
	protected void addPartyDocuments(WebDriver driver, ExtentTest step, String partyName, JSONArray partyDocuments)
			throws Exception {

		Iterator<Object> partyDocumentIterator = partyDocuments.iterator();

		driver.findElement(By.id(newDealLocator.getString("tab-PartyTabSet"))).findElements(By.tagName("li")).get(3)
				.click();
		...
		while (partyDocumentIterator.hasNext()) {
			...
			try {
				utility.addToExpectedDetails(expectedPartyDetails, partyName.toLowerCase().trim(),
						"document:" + partyDocument.getString("party_document_type").toLowerCase().trim());

				driver.findElement(By.id(newDealLocator.getString("button-NewPartyDocument"))).click();

				if (partyDocument.has("party_link_document")) {

					driver.findElement(By.id(newDealLocator.getString("checkbox-PartyDocumentCheck"))).click();
				...
				Thread.sleep(500);
				driver.findElement(By.id(newDealLocator.getString("select-PartyDocumentType")))
						.findElement(By.tagName("input")).click();
				utility.selectFromDivMenu(driver, step,
						driver.findElement(By.id(newDealLocator.getString("select-PartyDocumentType")))
								.findElement(By.tagName("ul")),
						partyDocument.getString("party_document_type"));

				Select partyDocumentNature = new Select(
						driver.findElement(By.xpath(newDealLocator.getString("select-PartyDocumentNature"))));
				partyDocumentNature.selectByVisibleText(partyDocument.getString("party_document_nature"));

				if (!utility.isElementHiddenNow(driver,
						By.id(newDealLocator.getString("input-PartyDocumentFromDate")))) {
					driver.findElement(By.id(newDealLocator.getString("input-PartyDocumentFromDate")))
							.findElement(By.tagName("input"))
							.sendKeys(partyDocument.getString("party_document_fromdate"));

					if (partyDocument.has("party_document_tilldate")) {
						driver.findElement(By.id(newDealLocator.getString("input-PartyDocumentTillDate")))
								.findElement(By.tagName("input"))
								.sendKeys(partyDocument.getString("party_document_tilldate"));
					}
				}

				driver.findElement(By.id(newDealLocator.getString("input-PartyDocumentDescription")))
						.sendKeys(partyDocument.getString("party_document_description"));
				...
				driver.findElement(By.id(newDealLocator.getString("input-PartyDocumentUpload")))
						.sendKeys(partyDocumentFile.getAbsolutePath());
				Thread.sleep(1000);
			} catch (Exception e) {
				step.log(Status.FAIL,
						"Error while adding Party Document: " + partyDocument.getString("party_document_type"));
				report.addStepFailScreenshot(driver, step);
				step.log(Status.FAIL, e.getMessage());
				throw new Exception(
						"Error while adding Party Document: " + partyDocument.getString("party_document_type"));
	...
	protected void addParties(WebDriver driver, ExtentTest step, JSONArray parties) throws Exception {
		...
		while (partyIterator.hasNext()) {
			...
			try {
				...
				utility.addToExpectedDetails(expectedPartyDetails, party.getString("party_name").toLowerCase().trim(),
						"responsibility:" + party.getString("party_responsibility").toLowerCase().trim());
				...
				if (party.getBoolean("party_live")) {
					...
					if (party.getString("party_type").toLowerCase().equals("external")) {
						...
						driver.findElement(By.id(newDealLocator.getString("checkbox-LinkPartyType")))
					...
					driver.findElement(By.id(newDealLocator.getString("input-LinkPartyName")))
							.sendKeys(party.getString("party_name"));

					driver.findElement(By.id(newDealLocator.getString("button-LinkPartySearch"))).click();
					utility.waitForElementToBeDisplayed(driver,
							By.xpath(newDealLocator.getString("checkbox-LinkPartySelect")), 2, 10);

					if (driver.findElements(By.xpath(newDealLocator.getString("checkbox-LinkPartySelect")))
							.size() > 1) {
						driver.findElements(By.xpath(newDealLocator.getString("checkbox-LinkPartySelect"))).get(1)
								.findElement(By.tagName("input")).click();
						partyNameSuggestions.add(driver
								.findElements(By.xpath(newDealLocator.getString("column-PartyName"))).get(2).getText());
					...
					if (party.getString("party_type").toLowerCase().equals("external")) {
						...
						driver.findElement(By.id(newDealLocator.getString("checkbox-PartyType")))
								.findElement(By.tagName("div")).click();
					}

					driver.findElement(By.id(newDealLocator.getString("select-PartyCustomerID")))
							.sendKeys(party.getString("party_customer_id"));
					driver.findElement(By.id(newDealLocator.getString("select-PartyName")))
							.sendKeys(party.getString("party_name"));

					driver.findElement(By.id(newDealLocator.getString("input-PartyRemarks")))
							.sendKeys(party.getString("party_remarks"));
				}

				driver.findElement(By.id(newDealLocator.getString("select-PartyResponsibility"))).click();
				Thread.sleep(1000);
				driver.findElement(By.xpath(newDealLocator.getString("select-resopnce"))).click();
				Thread.sleep(500);
				/*driver.findElement(By.id(newDealLocator.getString("select-PartyResponsibility"))).findElement(By.tagName("ul")).sendKeys(Keys.TAB);
				 * utility.selectFromDivMenu(driver, step,
				 * driver.findElement(By.id(newDealLocator.getString(
				 * "select-PartyResponsibility"))) .findElement(By.tagName("ul")),
				 * party.getString("party_responsibility"));
				 */
				...
				if (responsibilityList.contains(party.getString("party_responsibility").toLowerCase().trim())) {
					...
					List<WebElement> partyAttributes = driver
							.findElements(By.id(newDealLocator.getString("input-PartyAttributeValue")));
					partyAttributes.get(0).findElement(By.tagName("input"))
							.sendKeys(party.getString("party_attribute_name"));
...
//					partyAttributeCountry.selectByVisibleText(party.getString("party_attribute_country"));
					partyAttributes.get(1).findElement(By.tagName("input"))
							.sendKeys(party.getString("party_attribute_country"));
				}

				if (party.getBoolean("party_ecommerce")) {
					utility.addToExpectedDetails(expectedPartyDetails,
							party.getString("party_name").toLowerCase().trim(),
							"participant:" + party.getString("party_ecommerce_participant_id").toLowerCase().trim());
					driver.findElements(By.id(newDealLocator.getString("checkbox-PartyNeutral"))).get(1).click();

					driver.findElement(By.id(newDealLocator.getString("input-PartyEcommerceParticipantID")))
							.sendKeys(party.getString("party_ecommerce_participant_id"));

					Select partyEcommerceStatus = new Select(
							driver.findElements(By.id(newDealLocator.getString("select-PartyEcommerceStatus"))).get(1));
					partyEcommerceStatus.selectByVisibleText(party.getString("party_ecommerce_status"));

					List<WebElement> partyEcommerceDates = driver
							.findElements(By.id(newDealLocator.getString("input-DatePicker")));
					...
					if (party.has("party_ecommerce_debit_account")) {
						utility.addToExpectedDetails(expectedPartyDetails,
								party.getString("party_name").toLowerCase().trim(),
								"debit:" + party.getString("party_ecommerce_debit_account").toLowerCase().trim());

						driver.findElement(By.xpath(newDealLocator.getString("input-FilterText")))
								.sendKeys(party.getString("party_ecommerce_debit_account"));
						Thread.sleep(2000);
						driver.findElements(By.xpath(newDealLocator.getString("checkbox-PartyEcommerceAccount"))).get(1)
								.click();
					}
				...
				if (party.has("party_documents")) {
					addPartyDocuments(driver, step, party.getString("party_name"),
							party.getJSONArray("party_documents"));
				}
	...
	private void addIncomeBooking(WebDriver driver, ExtentTest step, JSONObject scheduledFee) throws Exception {
		try {
			utility.addToExpectedDetails(expectedFeeDetails,
					scheduledFee.getString("scheduled_fee_instruction_name").toLowerCase().trim(),
					"type:income booking");

			driver.findElement(By.id(newDealLocator.getString("input-IncomeBookingInstructionName"))).clear();
			driver.findElement(By.id(newDealLocator.getString("input-IncomeBookingInstructionName")))
					.sendKeys(scheduledFee.getString("scheduled_fee_instruction_name"));

			Select incomeBookingCurrency = new Select(
					driver.findElement(By.xpath(newDealLocator.getString("select-IncomeBookingCurrency"))));
			...
			if (scheduledFee.getBoolean("scheduled_fee_upfront")) {
				...
				driver.findElement(By.id(newDealLocator.getString("input-IncomeBookingUpfrontAmount")))
						.sendKeys(scheduledFee.getString("scheduled_fee_upfront_amount"));
			}

			Select incomeBookingFeeMode = new Select(
					driver.findElement(By.xpath(newDealLocator.getString("select-IncomeBookingFeeMode"))));
			incomeBookingFeeMode.selectByVisibleText(scheduledFee.getString("fee_mode"));

			Select incomeBookingFeeType = new Select(
					driver.findElement(By.id(newDealLocator.getString("select-IncomeBookingFeeType")))
							.findElement(By.tagName("select")));
			incomeBookingFeeType.selectByVisibleText(scheduledFee.getString("fee_type"));

			switch (scheduledFee.getString("fee_type")) {
			case "Recurring":
				driver.findElement(By.id(newDealLocator.getString("input-IncomeBookingRecurringAmount")))
						.sendKeys(scheduledFee.getString("scheduled_fee_recurring_amount"));
				break;

			case "Transaction Based":
				Select incomeBookingTransactionBased = new Select(
						driver.findElement(By.id(newDealLocator.getString("select-IncomeBookingTransactionBased")))
								.findElement(By.tagName("select")));
				incomeBookingTransactionBased
						.selectByVisibleText(scheduledFee.getString("scheduled_fee_transaction_based"));

				driver.findElement(By.id(newDealLocator.getString("select-IncomeBookingFeeReferenceAccount")))
						.findElement(By.tagName("input")).click();
				utility.selectFromDivMenu(driver, step,
						driver.findElement(By.id(newDealLocator.getString("select-IncomeBookingFeeReferenceAccount")))
								.findElement(By.tagName("ul")),
						scheduledFee.getString("scheduled_fee_reference_account"));
				break;

			case "% of EOD Balance":
				driver.findElement(By.id(newDealLocator.getString("input-IncomeBookingEODBalancePercent")))
						.sendKeys(scheduledFee.getString("scheduled_fee_eod_balance_percent"));
			...
			driver.findElement(By.id(newDealLocator.getString("input-IncomeBookingRemarks")))
					.sendKeys(scheduledFee.getString("scheduled_fee_remarks"));

			Select incomeBookingHolidayAction = new Select(
					driver.findElement(By.id(newDealLocator.getString("select-IncomeBookingHolidayAction")))
							.findElement(By.tagName("select")));
			...
			if (scheduledFee.getString("fee_type").equals("Transaction Based")) {
				...
				if (scheduledFee.getString("scheduled_fee_transaction_based").equals("Value")) {
					Select scheduledFeeTierFeeAmountType = new Select(driver
							.findElement(By.id(newDealLocator.getString("select-IncomeBookingTiersFeeAmountType")))
							.findElement(By.tagName("select")));
					scheduledFeeTierFeeAmountType
							.selectByVisibleText(scheduledFee.getString("scheduled_fee_tiers_fee_amount_type"));
				}
				...
				while (scheduledFeeTiers.hasNext()) {
					...
					if (scheduledFeeTier.has("scheduled_fee_tiers_payment_instrument")) {
						driver.findElement(
								By.id(newDealLocator.getString("select-IncomeBookingTiersPaymentInstrument")))
								.findElement(By.tagName("input")).click();
						utility.selectFromDivMenu(driver, step,
								driver.findElement(
										By.id(newDealLocator.getString("select-IncomeBookingTiersPaymentInstrument")))
										.findElement(By.tagName("ul")),
								scheduledFeeTier.getString("scheduled_fee_tiers_payment_instrument"));

						driver.findElement(By.xpath(newDealLocator.getString("button-IncomeBookingTiersAddTier")))
								.click();
					} else {
						driver.findElement(By.xpath(
								newDealLocator.getString("button-IncomeBookingTiersAddTierWithoutPaymentInstrument")))
								.click();
					}

					Iterator<Object> scheduledFeeTiersDetails = scheduledFeeTier
							.getJSONArray("scheduled_fee_tiers_details").iterator();
					...
					while (scheduledFeeTiersDetails.hasNext()) {
						...
						WebElement scheduledFeeTiersTable = driver
								.findElements(By.xpath(newDealLocator.getString("table-IncomeBookingTiers")))
								.get(scheduledFeeTierIndex).findElement(By.tagName("tbody"));
						if (scheduledFeeTiersDetailIndex > 0) {
							scheduledFeeTiersTable.findElement(By.tagName("tr")).findElements(By.tagName("td")).get(4)
									.findElement(By.tagName("span")).click();
						}
						WebElement scheduledFeeTiersTableRow = scheduledFeeTiersTable.findElements(By.tagName("tr"))
								.get(scheduledFeeTiersDetailIndex);
						scheduledFeeTiersTableRow.findElements(By.tagName("td")).get(1).findElement(By.tagName("input"))
								.sendKeys(scheduledFeeTiersDetail.getString("scheduled_fee_tiers_cumulative_from"));
						scheduledFeeTiersTableRow.findElements(By.tagName("td")).get(2).findElement(By.tagName("input"))
								.sendKeys(scheduledFeeTiersDetail.getString("scheduled_fee_tiers_cumulative_to"));
						scheduledFeeTiersTableRow.findElements(By.tagName("td")).get(3).findElement(By.tagName("input"))
								.sendKeys(scheduledFeeTiersDetail.getString("scheduled_fee_tiers_fee"));
			...
			if (scheduledFee.getString("fee_type").equals("Recurring")) {
				Select incomeBookingContributeAs = new Select(
						driver.findElement(By.id(newDealLocator.getString("select-IncomeBookingContributeAs")))
								.findElement(By.tagName("select")));
				incomeBookingContributeAs.selectByVisibleText(scheduledFee.getString("scheduled_fee_contribute_as"));
			}

			Select incomeBookingTaxCategory = new Select(
					driver.findElement(By.id(newDealLocator.getString("select-IncomeBookingTaxCategory")))
							.findElement(By.tagName("select")));
			...
			if (scheduledFee.getJSONArray("scheduled_fee_parties").length() > 1) {
				List<WebElement> incomeBookingPartyList = driver
						.findElement(By.tagName(newDealLocator.getString("list-IncomeBookingParty")))
			...
			while (incomeBookingParties.hasNext()) {
				...
				driver.findElements(By.xpath(newDealLocator.getString("tab-IncomeBookingParty"))).get(noOfParties)
						.click();

				driver.findElement(By.id(newDealLocator.getString("select-IncomeBookingParty")))
						.findElement(By.tagName("input")).click();
				utility.selectFromDivMenu(driver, step,
						driver.findElement(By.id(newDealLocator.getString("select-IncomeBookingParty")))
								.findElement(By.tagName("ul")),
						incomeBookingParty.getString("party_name"));
				Thread.sleep(500);

				if (incomeBookingParty.has("party_fee_debit_account")) {
					driver.findElement(By.id(newDealLocator.getString("select-IncomeBookingFeeDebitAccount")))
							.findElement(By.tagName("input")).click();
					utility.selectFromDivMenu(driver, step,
							driver.findElement(By.id(newDealLocator.getString("select-IncomeBookingFeeDebitAccount")))
									.findElement(By.tagName("ul")),
							incomeBookingParty.getString("party_fee_debit_account"));
				}

				if (scheduledFee.getBoolean("scheduled_fee_upfront")) {
					driver.findElement(By.xpath(newDealLocator.getString("input-IncomeBookingUpfrontContribution")))
							.sendKeys(incomeBookingParty.getString("party_upfront_contribution"));
				}

				driver.findElement(By.id(newDealLocator.getString("input-IncomeBookingFeeContribution")))
						.sendKeys(incomeBookingParty.getString("party_fee_contribution"));
				driver.findElement(By.id(newDealLocator.getString("input-IncomeBookingTaxReferenceNo")))
						.sendKeys(incomeBookingParty.getString("party_tax_reference_no"));
				driver.findElement(By.xpath(newDealLocator.getString("input-IncomeBookingInvoiceTo")))
						.sendKeys(incomeBookingParty.getString("party_invoice_to"));
			...
			if (scheduledFee.getBoolean("scheduled_fee_upfront")) {
				driver.findElement(By.id(newDealLocator.getString("input-IncomeBookingScheduleExecutionDate")))
						.findElement(By.tagName("input"))
						.sendKeys(scheduledFee.getString("scheduled_fee_upfront_execution_date"));
				Select incomeBookingScheduledAt = new Select(driver
						.findElements(By.id(newDealLocator.getString("select-ScheduledPaymentScheduledAt"))).get(1));
				incomeBookingScheduledAt
						.selectByVisibleText(scheduledFee.getString("scheduled_fee_upfront_scheduled_at"));

				driver.findElement(By.tagName(newDealLocator.getString("radio-IncomeBookingUpfrontSchedule")))
						.findElements(By.tagName("label")).get(1).click();
			}

			driver.findElement(By.id(newDealLocator.getString("input-IncomeBookingScheduleStartDate")))
					.findElement(By.tagName("input")).sendKeys(scheduledFee.getString("scheduled_fee_start_date"));
			driver.findElement(By.id(newDealLocator.getString("input-IncomeBookingScheduleEndDate")))
					.findElement(By.tagName("input")).sendKeys(scheduledFee.getString("scheduled_fee_end_date"));
			...
			if (scheduledFee.getBoolean("scheduled_fee_invoice_same_as_scheduled_date")) {
				...
				driver.findElement(By.xpath(newDealLocator.getString("input-IncomeBookingInvoiceDueDate")))
						.sendKeys(scheduledFee.getString("scheduled_fee_invoice_due_date"));
			}

			driver.findElement(By.xpath(newDealLocator.getString("input-IncomeBookingFeeDueDate")))
					.sendKeys(scheduledFee.getString("scheduled_fee_due_date"));

			Select incomeBookingScheduledAt = new Select(
					driver.findElements(By.id(newDealLocator.getString("select-ScheduledPaymentScheduledAt"))).get(1));
			incomeBookingScheduledAt.selectByVisibleText(scheduledFee.getString("scheduled_fee_scheduled_at"));

			Select incomeBookingFrequency = new Select(
					driver.findElements(By.id(newDealLocator.getString("select-ScheduledPaymentFrequency"))).get(1));
			...
			driver.findElement(By.id(newDealLocator.getString("input-ScheduledPaymentRepeatEvery")))
					.sendKeys(scheduledFee.getString("scheduled_fee_repeat_every"));
			...
			case "monthly":
				...
				Select incomeBookingFrequencySubtype = new Select(
						driver.findElements(By.id(newDealLocator.getString("select-ScheduledPaymentSubType"))).get(1));
				...
				if (scheduledFee.getString("scheduled_fee_sub_type").toLowerCase().equals("day of month")) {
					...
					driver.findElement(By.id(newDealLocator.getString("select-ScheduledPaymentDays")))
							.findElement(By.tagName("input")).click();
					utility.selectFromDivMenu(driver, step,
							driver.findElement(By.xpath(newDealLocator.getString("select-ChildProcessingUnit"))),
							scheduledFee.getJSONArray("scheduled_fee_days"));
				} else if (scheduledFee.getString("scheduled_fee_sub_type").toLowerCase().equals("weekday(s)")) {

					Select incomeBookingFrequencyWeek = new Select(
							driver.findElements(By.id(newDealLocator.getString("select-ScheduledPaymentWeek"))).get(1));
			...
			List<WebElement> incomeBookingNarration = driver
					.findElements(By.id(newDealLocator.getString("input-IncomeBookingNarration")));
			if (scheduledFee.has("scheduled_fee_credit_account")) {
				Select incomeBookingFeeCreditAccountNo = new Select(
						driver.findElement(By.id(newDealLocator.getString("select-IncomeBookingFeeCreditAccountNo")))
								.findElement(By.tagName("select")));
				incomeBookingFeeCreditAccountNo
						.selectByVisibleText(scheduledFee.getString("scheduled_fee_credit_account"));
			...
			if (scheduledFee.has("scheduled_fee_tax_credit_account")) {
				Select incomeBookingTaxCreditAccountNo = new Select(
						driver.findElement(By.id(newDealLocator.getString("select-IncomeBookingTaxCreditAccountNo")))
								.findElement(By.tagName("select")));
				incomeBookingTaxCreditAccountNo
						.selectByVisibleText(scheduledFee.getString("scheduled_fee_tax_credit_account"));
			...
			if (scheduledFee.has("scheduled_fee_invoice_contacts")) {
				...
				step.log(Status.INFO,
						"Contacts available at Income Booking - Expected: " + expectedContactList.toString()
								+ " Actual: " + actualContactList.toString() + " (See below image)");
				report.addStepInfoScreenshot(driver, step);
			...
			if (scheduledFee.getBoolean("scheduled_fee_due_reminder")) {
				...
				driver.findElement(By.id(newDealLocator.getString("input-IncomeBookingNoOfDaysBeforeCollecting")))
						.sendKeys(scheduledFee.getString("scheduled_fee_no_of_days_before_collecting"));
			}

			if (scheduledFee.getBoolean("scheduled_fee_overdue_reminder")) {
				utility.focusElement(driver,
						driver.findElement(By.id(newDealLocator.getString("button-IncomeBookingReminderContacts"))));
				driver.findElement(By.id(newDealLocator.getString("checkbox-IncomeBookingOverdueFeesReminder")))
						.click();

				Select incomeBookingOverdueReminderFrequency = new Select(driver
						.findElements(By.id(newDealLocator.getString("select-ScheduledPaymentFrequency"))).get(1));
				incomeBookingOverdueReminderFrequency
						.selectByVisibleText(scheduledFee.getString("scheduled_fee_overdue_reminder_frequency"));

				if (scheduledFee.getString("scheduled_fee_overdue_type").equals("Forever")) {
					driver.findElement(By.xpath(newDealLocator.getString("label-IncomeBookingOverdueTypeForver")))
							.click();
				} else if (scheduledFee.getString("scheduled_fee_overdue_type").equals("Stop after")) {
					driver.findElement(By.xpath(newDealLocator.getString("label-IncomeBookingOverdueTypeStopAfter")))
							.click();
					driver.findElement(By.id(newDealLocator.getString("input-IncomeBookingOverdueNoOfReminders")))
							.sendKeys(scheduledFee.getString("scheduled_fee_no_of_reminders"));
				}

				driver.findElement(By.id(newDealLocator.getString("input-IncomeBookingOverdueRepeatEvery"))).clear();
				driver.findElement(By.id(newDealLocator.getString("input-IncomeBookingOverdueRepeatEvery")))
						.sendKeys(scheduledFee.getString("scheduled_fee_overdue_reminder_repeat_every"));
				...
				case "weekly":
					...
					selectDayOfWeek(driver, step,
							scheduledFee.getJSONArray("scheduled_fee_overdue_reminder_days_of_week"));
					break;

				case "monthly":

					Select incomeBookingFrequencySubtype = new Select(driver
							.findElements(By.id(newDealLocator.getString("select-ScheduledPaymentSubType"))).get(1));
					incomeBookingFrequencySubtype
							.selectByVisibleText(scheduledFee.getString("scheduled_fee_overdue_reminder_sub_type"));

					if (scheduledFee.getString("scheduled_fee_overdue_reminder_sub_type").toLowerCase()
							.equals("day of month")) {
						...
						utility.selectFromDivMenu(driver, step,
								driver.findElement(By.id(newDealLocator.getString("select-ScheduledPaymentDays")))
										.findElement(By.tagName("ul")),
								scheduledFee.getJSONArray("scheduled_fee_overdue_reminder_days"));
					} else if (scheduledFee.getString("scheduled_fee_overdue_reminder_sub_type").toLowerCase()
							.equals("weekday(s)")) {

						Select incomeBookingFrequencyWeek = new Select(driver
								.findElements(By.id(newDealLocator.getString("select-ScheduledPaymentWeek"))).get(1));
						incomeBookingFrequencyWeek
								.selectByVisibleText(scheduledFee.getString("scheduled_fee_overdue_reminder_week"));
				...
				case "yearly":
					...
					Select incomeBookingFrequencyYearlySubtype = new Select(driver
							.findElements(By.id(newDealLocator.getString("select-ScheduledPaymentSubType"))).get(1));
					incomeBookingFrequencyYearlySubtype
							.selectByVisibleText(scheduledFee.getString("scheduled_fee_overdue_reminder_sub_type"));

					if (scheduledFee.getString("scheduled_fee_overdue_reminder_sub_type").toLowerCase()
							.equals("day of month")) {

						Select incomeBookingFrequencyMonth = new Select(
								driver.findElement(By.id(newDealLocator.getString("select-IncomeBookingOverdueMonth")))
										.findElement(By.tagName("select")));
						incomeBookingFrequencyMonth
								.selectByVisibleText(scheduledFee.getString("scheduled_fee_overdue_reminder_month"));
				...
				default:
					step.log(Status.FAIL,
							"Invalid fee frequency: " + scheduledFee.getString("scheduled_fee_frequency"));
					throw new Exception("Invalid fee frequency: " + scheduledFee.getString("scheduled_fee_frequency"));
				}
			}
			...
			if (!utility.isElementHiddenNow(driver, By.id(newDealLocator.getString("label-Title")))) {
				actualResponseMessages
						.add(utility.extractTitleMessage(driver, step, By.id(newDealLocator.getString("label-Title"))));
				driver.findElement(By.xpath(newDealLocator.getString("button-SubmitDealOK"))).click();
			}
			utility.waitForProgressBarToLoad(driver);
		} catch (Exception e) {
	...
	private void addScheduledFees(WebDriver driver, ExtentTest step, JSONArray scheduledFees) throws Exception {
		...
		while (scheduledFeeIterator.hasNext()) {
			...
			try {
				...
				default:
					step.log(Status.FAIL,
							"Invalid Scheduled Fees Type: " + scheduledFee.getString("scheduled_fees_type"));
					report.addStepFailScreenshot(driver, step);
					throw new Exception(
							"Invalid Scheduled Fees Type: " + scheduledFee.getString("scheduled_fees_type"));
				}
				...
				step.log(Status.FAIL,
						"Error while adding scheduled fees of type: " + scheduledFee.getString("scheduled_fees_type"));
				report.addStepFailScreenshot(driver, step);
				step.log(Status.FAIL, e.getMessage());
				throw new Exception(
						"Error while adding scheduled fees of type: " + scheduledFee.getString("scheduled_fees_type"));
	...
	private void addEntitlementDeal(WebDriver driver, ExtentTest step, String dealName, JSONObject entitlement)
			throws Exception {

		try {
			utility.selectFromDivMenu(driver, step,
					driver.findElement(By.id(newDealLocator.getString("select-EntitlementTab"))), "DEAL");
			utility.addToExpectedDetails(expectedDealDetails, dealName.toLowerCase().trim(),
					"entitlement:" + entitlement.getJSONArray("entitlement_initiating_contacts").toList().toString());
			driver.findElement(By.id(newDealLocator.getString("button-NewEntitlement"))).click();

			Select entitlementCurrency = new Select(
					driver.findElement(By.xpath(newDealLocator.getString("select-EntitlementCurrency"))));
			...
			if (entitlement.has("entitlement_ranges_from")) {
				driver.findElement(By.id(newDealLocator.getString("input-EntitlementRangesFrom")))
						.sendKeys(entitlement.getString("entitlement_ranges_from"));
			}

			if (entitlement.has("entitlement_ranges_to")) {
				driver.findElement(By.id(newDealLocator.getString("input-EntitlementRangesTo")))
						.sendKeys(entitlement.getString("entitlement_ranges_to"));
			}
			...
			step.log(Status.INFO,
					"Transation Initiating Contacts: "
							+ entitlement.getJSONArray("entitlement_initiating_contacts").toList().toString()
							+ " added to deal");
	...
	private void addEntitlementAccount(WebDriver driver, ExtentTest step, JSONObject entitlement) throws Exception {
		try {
			utility.selectFromDivMenu(driver, step,
					driver.findElement(By.id(newDealLocator.getString("select-EntitlementTab"))), "ACCOUNT");
			utility.addToExpectedDetails(expectedAccountDetails,
					entitlement.getString("entitlement_source_account").split("-")[0].toLowerCase().trim(),
					"entitlement:" + entitlement.getJSONArray("entitlement_initiating_contacts").toList().toString());
			driver.findElement(By.id(newDealLocator.getString("button-NewEntitlement"))).click();

			driver.findElement(By.id(newDealLocator.getString("select-EntitlementSourceAccount")))
					.findElement(By.tagName("input")).click();
			List<String> actualSourceAccounts = utility.extractOptionsFromSelect(driver, step,
					driver.findElement(By.id(newDealLocator.getString("select-EntitlementSourceAccount")))
							.findElement(By.tagName("ul")));
			step.log(Status.INFO, "Source Accounts available at Entitlement Account - Expected: "
					+ expectedSourceAccounts.toString() + " Actual: " + actualSourceAccounts.toString());
			report.addStepInfoScreenshot(driver, step);
			utility.selectFromDivMenu(driver, step,
					driver.findElement(By.id(newDealLocator.getString("select-EntitlementSourceAccount")))
							.findElement(By.tagName("ul")),
					entitlement.getString("entitlement_source_account"));
			...
			step.log(Status.INFO,
					"Transation Initiating Contacts: "
							+ entitlement.getJSONArray("entitlement_initiating_contacts").toList().toString()
							+ " added to account");
			// List<String> entitlementInitiatingContacts =
			// utility.extractColumnsFromTable(By.id(newDealLocator.getString("table-EntitlementList")),
			// 2);
	...
	private void addEntitlements(WebDriver driver, ExtentTest step, String dealName, JSONArray entitlements)
			throws Exception {
	...
	private void addBudgetConsolidated(WebDriver driver, ExtentTest step, JSONObject budget) throws Exception {
		...
		try {
			...
			if (budget.getBoolean("budget_carry_forward")) {
				driver.findElement(By.id(newDealLocator.getString("checkbox-BudgetCarryForward")))
						.findElement(By.tagName("span")).click();
			}

			Select budgetInterval = new Select(
					driver.findElements(By.id(newDealLocator.getString("select-BudgetInterval"))).get(1));
			budgetInterval.selectByVisibleText(budget.getString("budget_interval"));

			Select budgetDuration = new Select(
					driver.findElements(By.id(newDealLocator.getString("select-BudgetDuration"))).get(1));
			budgetDuration.selectByVisibleText(budget.getString("budget_duration"));

			driver.findElement(By.id(newDealLocator.getString("input-BudgetAllocated")))
					.sendKeys(budget.getString("budget_allocated_amount"));
...
//			budgetDatePicker.get(1).sendKeys(budget.getString("budget_ends_on"));
	...
	private void addBudgetPurpose(WebDriver driver, ExtentTest step, JSONObject budget) throws Exception {
		...
		try {
			...
			driver.findElement(By.id(newDealLocator.getString("input-BudgetPurpose")))
					.sendKeys(budget.getString("budget_purpose"));
...
//			budgetDatePicker.get(1).sendKeys(budget.getString("budget_ends_on"));
	...
	private void addBudgetDestination(WebDriver driver, ExtentTest step, JSONObject budget) throws Exception {
		...
		try {
			...
			driver.findElement(By.id(newDealLocator.getString("input-BudgetDestination")))
					.sendKeys(budget.getString("budget_destination"));
			List<String> actualSourceAccounts = utility.extractOptionsFromSelect(driver, step,
					driver.findElement(By.xpath(newDealLocator.getString("select-BudgetDestination"))));
	...
	private void addBudgetPurposeAndDestination(WebDriver driver, ExtentTest step, JSONObject budget) throws Exception {
		try {
			...
			List<String> actualSourceAccounts = utility.extractOptionsFromSelect(driver, step,
					driver.findElement(By.xpath(newDealLocator.getString("select-BudgetDestination"))));
			step.log(Status.INFO,
					"Source Account available at Budget Purpose and Destination: " + actualSourceAccounts.toString());
			report.addStepInfoScreenshot(driver, step);
...
//			budgetDatePicker.get(1).sendKeys(budget.getString("budget_ends_on"));
			...
			step.log(Status.FAIL, "Error while adding Budget - Purpose: " + budget.getString("budget_purpose")
					+ " and Destination: " + budget.getString("budget_destination"));
			report.addStepFailScreenshot(driver, step);
			step.log(Status.FAIL, e.getMessage());
			throw new Exception("Error while adding Budget - Purpose: " + budget.getString("budget_purpose")
					+ " and Destination: " + budget.getString("budget_destination"));
	...
	protected void addBudgetGroups(WebDriver driver, ExtentTest step, JSONArray budgetGroups) throws Exception {
		...
		while (budgetGroupIterator.hasNext()) {
			...
			try {
				utility.addToExpectedDetails(expectedAccountDetails,
						budgetGroup.getString("budget_group_source_account").split("-")[0].toLowerCase().trim(),
						"name:" + budgetGroup.getString("budget_group_name").toLowerCase() + ";" + "source:"
								+ budgetGroup.getString("budget_group_source_account").split("-")[0].toLowerCase()
										.trim()
								+ ";" + "budgets configured: " + budgetGroup.getJSONArray("budgets").length());

				driver.findElement(By.id(newDealLocator.getString("button-NewBudgetGroup"))).click();

				driver.findElement(By.id(newDealLocator.getString("input-BudgetGroupName")))
						.sendKeys(budgetGroup.getString("budget_group_name"));

				Thread.sleep(500);
				driver.findElement(By.id(newDealLocator.getString("select-BudgetGroupSourceAccount")))
						.findElement(By.tagName("input")).click();
				List<String> actualSourceAccounts = utility.extractOptionsFromSelect(driver, step,
						driver.findElement(By.id(newDealLocator.getString("select-BudgetGroupSourceAccount")))
								.findElement(By.tagName("ul")));
				step.log(Status.INFO, "Source Accounts available at Budget Group - Expected: "
						+ expectedSourceAccounts.toString() + " Actual: " + actualSourceAccounts.toString());
				report.addStepInfoScreenshot(driver, step);
				utility.selectFromDivMenu(driver, step,
						driver.findElement(By.id(newDealLocator.getString("select-BudgetGroupSourceAccount")))
								.findElement(By.tagName("ul")),
						budgetGroup.getString("budget_group_source_account"));

				Select budgetGroupFiscalMonth = new Select(
						driver.findElements(By.id(newDealLocator.getString("select-BudgetGroupFiscalMonth"))).get(1));
				budgetGroupFiscalMonth.selectByVisibleText(budgetGroup.getString("budget_group_fiscal_month"));

				Select budgetGroupFiscalYear = new Select(
						driver.findElements(By.id(newDealLocator.getString("select-BudgetGroupFiscalYear"))).get(1));
				...
				while (budgetIterator.hasNext()) {
					...
					try {
						if (incrementor == 0) {
							driver.findElement(By.id(newDealLocator.getString("button-NewBudget")))
									.findElement(By.tagName("i")).click();
						} else {
	...
	private void selectDayOfWeek(WebDriver driver, ExtentTest step, JSONArray daysOfWeek) {
		...
		Map<String, Integer> dayOfWeek = Map.of("Sunday", 0, "Monday", 1, "Tuesday", 2, "Wednesday", 3, "Thursday", 4,
				"Friday", 5, "Saturday", 6);
		...
		while (daysOfWeekIterator.hasNext()) {
			...
			driver.findElement(By.id(newDealLocator.getString("card-ScheduledPaymentWeekSelect")))
					.findElement(By.tagName("div")).findElements(By.tagName("div")).get(dayOfWeek.get(day)).click();
		}

	}

	private void addBTSubInstruction(WebDriver driver, ExtentTest step, WebElement datePicker,
			JSONObject subInstruction) throws Exception {

		try {

			driver.findElement(By.id(newDealLocator.getString("input-ScheduledPaymentDebitAccountBICCode")))
					.sendKeys(subInstruction.getString("sub_instruction_debit_account_BIC_code"));

			Select subInstructionTransactionChargesBearedBy = new Select(driver
					.findElement(By.id(newDealLocator.getString("select-ScheduledPaymentTransactionChargesBearedBy"))));
			subInstructionTransactionChargesBearedBy
					.selectByVisibleText(subInstruction.getString("sub_instruction_transaction_charges_beared_by"));

			driver.findElement(By.id(newDealLocator.getString("input-ScheduledPaymentAmount")))
					.sendKeys(subInstruction.getString("sub_instruction_amount"));

			Select subInstructionPaymentCurrency = new Select(
					driver.findElement(By.id(newDealLocator.getString("select-ScheduledPaymentCurrency"))));
			...
			driver.findElement(By.id(newDealLocator.getString("input-ScheduledPaymentBeneficiaryName")))
					.sendKeys(subInstruction.getString("sub_instruction_beneficiary_name"));

			driver.findElement(By.id(newDealLocator.getString("input-ScheduledPaymentBeneficiaryAddress")))
					.sendKeys(subInstruction.getString("sub_instruction_beneficiary_address"));
		} catch (Exception e) {
	...
	private void addScheduledPaymentRetentionSurplus(WebDriver driver, ExtentTest step, JSONObject scheduledInstruction)
			throws Exception {
		try {

			utility.addToExpectedDetails(expectedAccountDetails,
					scheduledInstruction.getString("scheduled_payment_source_account").split("-")[0].toLowerCase()
							.trim(),
					"name:" + scheduledInstruction.getString("scheduled_payment_name").toLowerCase().trim() + ";"
							+ "purpose:"
							+ scheduledInstruction.getString("scheduled_payment_purpose").toLowerCase().trim() + ";"
							+ "type:payment-retention-surplus");

			driver.findElement(By.id(newDealLocator.getString("input-ScheduledPaymentName"))).clear();
			driver.findElement(By.id(newDealLocator.getString("input-ScheduledPaymentName")))
					.sendKeys(scheduledInstruction.getString("scheduled_payment_name"));

			Select scheduledPaymentPurpose = new Select(
					driver.findElements(By.id(newDealLocator.getString("select-ScheduledPaymentPurpose"))).get(1));
			scheduledPaymentPurpose.selectByVisibleText(scheduledInstruction.getString("scheduled_payment_purpose"));

			Thread.sleep(500);
			driver.findElement(By.id(newDealLocator.getString("select-ScheduledPaymentSourceAccount")))
					.findElement(By.tagName("input")).click();
			List<String> actualSourceAccounts = utility.extractOptionsFromSelect(driver, step,
					driver.findElement(By.id(newDealLocator.getString("select-ScheduledPaymentSourceAccount")))
							.findElement(By.tagName("ul")));
			step.log(Status.INFO, "Source Accounts available at Scheduled Instruction - Expected: "
					+ expectedSourceAccounts.toString() + " Actual: " + actualSourceAccounts.toString());
			report.addStepInfoScreenshot(driver, step);
			utility.selectFromDivMenu(driver, step,
					driver.findElement(By.id(newDealLocator.getString("select-ScheduledPaymentSourceAccount")))
							.findElement(By.tagName("ul")),
					scheduledInstruction.getString("scheduled_payment_source_account"));

			Select scheduledPaymentBalanceConsideration = new Select(
					driver.findElements(By.id(newDealLocator.getString("select-ScheduledPaymentBalanceConsideration")))
							.get(1));
			scheduledPaymentBalanceConsideration
					.selectByVisibleText(scheduledInstruction.getString("scheduled_payment_balance_consideration"));
			...
			if (scheduledInstruction.getBoolean("scheduled_payment_split")) {
				...
				Select scheduledPaymentSpecifyAmountAs = new Select(
						driver.findElement(By.id(newDealLocator.getString("select-ScheduledPaymentSpecifyAmountAs")))
								.findElement(By.tagName("select")));
				scheduledPaymentSpecifyAmountAs
						.selectByVisibleText(scheduledInstruction.getString("scheduled_payment_specify_amount_as"));

				driver.findElement(By.id(newDealLocator.getString("input-ScheduledPaymentValue")))
						.sendKeys(scheduledInstruction.getString("scheduled_payment_value"));
			}

			else if (scheduledInstruction.getBoolean("scheduled_payment_partial_payment")) {
				driver.findElement(By.id(newDealLocator.getString("checkbox-ScheduledPaymentPartialPayment")))
						.findElement(By.tagName("span")).click();
			}
			...
			if (scheduledInstruction.getBoolean("scheduled_payment_sweep_in")) {
				driver.findElement(By.id(newDealLocator.getString("checkbox-ScheduledPaymentSweepIn")))
						.findElement(By.tagName("div")).click();
			}
			...
			if (scheduledInstruction.getBoolean("scheduled_payment_repeating")) {
				...
				driver.findElement(By.id(newDealLocator.getString("input-ScheduledPaymentStartDate")))
						.findElement(By.tagName("input"))
						.sendKeys(scheduledInstruction.getString("scheduled_payment_start_date"));
				driver.findElement(By.id(newDealLocator.getString("input-ScheduledPaymentEndDate")))
						.findElement(By.tagName("input"))
						.sendKeys(scheduledInstruction.getString("scheduled_payment_end_date"));

				if (scheduledInstruction.getBoolean("scheduled_payment_all_sub_instructions")) {

					Select scheduledInstructionFrequency = new Select(driver
							.findElements(By.id(newDealLocator.getString("select-ScheduledPaymentFrequency"))).get(1));
					scheduledInstructionFrequency
							.selectByVisibleText(scheduledInstruction.getString("scheduled_payment_frequency"));

					driver.findElement(By.id(newDealLocator.getString("input-ScheduledPaymentRepeatEvery"))).clear();
					driver.findElement(By.id(newDealLocator.getString("input-ScheduledPaymentRepeatEvery")))
							.sendKeys(scheduledInstruction.getString("scheduled_payment_repeat_every"));
					...
					case "weekly":
						...
						selectDayOfWeek(driver, step,
								scheduledInstruction.getJSONArray("scheduled_payment_days_of_week"));
						break;

					case "monthly":

						Select scheduledInstructionFrequencySubtype = new Select(
								driver.findElements(By.id(newDealLocator.getString("select-ScheduledPaymentSubType")))
										.get(1));
						scheduledInstructionFrequencySubtype
								.selectByVisibleText(scheduledInstruction.getString("scheduled_payment_sub_type"));

						if (scheduledInstruction.getString("scheduled_payment_sub_type").toLowerCase()
								.equals("day of month")) {
							...
							utility.selectFromDivMenu(driver, step,
									driver.findElement(By.id(newDealLocator.getString("select-ScheduledPaymentDays")))
											.findElement(By.tagName("ul")),
									scheduledInstruction.getJSONArray("scheduled_payment_days"));
						} else if (scheduledInstruction.getString("scheduled_payment_sub_type").toLowerCase()
								.equals("weekday(s)")) {

							Select scheduledInstructionFrequencyWeek = new Select(
									driver.findElements(By.id(newDealLocator.getString("select-ScheduledPaymentWeek")))
											.get(1));
							scheduledInstructionFrequencyWeek
									.selectByVisibleText(scheduledInstruction.getString("scheduled_payment_week"));
					...
					default:
						step.log(Status.FAIL, "Invalid payment frequency: "
								+ scheduledInstruction.getString("scheduled_payment_frequency"));
						throw new Exception("Invalid payment frequency: "
								+ scheduledInstruction.getString("scheduled_payment_frequency"));
					}
				} else {
					driver.findElement(By.id(newDealLocator.getString("checkbox-ScheduledPaymentAllSubInstructions")))
							.findElement(By.tagName("input")).click();
				}
			} else {
				driver.findElement(By.id(newDealLocator.getString("checkbox-ScheduledPaymentRepeating")))
						.findElement(By.tagName("span")).click();
				driver.findElement(By.id(newDealLocator.getString("input-ScheduledPaymentExecutionDate")))
						.findElement(By.tagName("span"))
						.sendKeys(scheduledInstruction.getString("scheduled_payment_execution_date"));
			}

			Select scheduledInstructionScheduledAt = new Select(
					driver.findElements(By.id(newDealLocator.getString("select-ScheduledPaymentScheduledAt"))).get(1));
			scheduledInstructionScheduledAt
					.selectByVisibleText(scheduledInstruction.getString("scheduled_payment_scheduled_at"));

			Select scheduledInstructionHolidayAction = new Select(driver
					.findElements(By.id(newDealLocator.getString("select-ScheduledPaymentHolidayAction"))).get(1));
			scheduledInstructionHolidayAction
					.selectByVisibleText(scheduledInstruction.getString("scheduled_payment_holiday_action"));
			...
			if (scheduledInstruction.has("scheduled_payment_sub_instructions")) {
				...
				Iterator<Object> paymentSubInstructionsIterator = scheduledInstruction
						.getJSONArray("scheduled_payment_sub_instructions").iterator();

				driver.findElement(By.id(newDealLocator.getString("select-ScheduledPaymentBudget")))
						.findElement(By.tagName("input")).click();
				utility.selectFromDivMenu(driver, step,
						driver.findElement(By.id(newDealLocator.getString("select-ScheduledPaymentBudget")))
								.findElement(By.tagName("ul")),
						scheduledInstruction.getString("scheduled_instruction_budget"));

				if (scheduledInstruction.getBoolean("scheduled_payment_split")
						&& scheduledInstruction.getBoolean("scheduled_instruction_split_by_percentage")) {
					driver.findElement(By.id(newDealLocator.getString("checkbox-ScheduledPaymentSplitByPercentage")))
							.findElement(By.tagName("span")).click();
				}
				...
				while (paymentSubInstructionsIterator.hasNext()) {
					...
					driver.findElement(By.id(newDealLocator.getString("select-ScheduledPaymentInstrument")))
							.findElement(By.tagName("input")).click();
					utility.selectFromDivMenu(driver, step,
							driver.findElement(By.id(newDealLocator.getString("select-ScheduledPaymentInstrument")))
									.findElement(By.tagName("ul")),
							paymentSubInstruction.getString("sub_instruction_instrument"));

					Thread.sleep(500);
					if (!utility.isElementHiddenNow(driver,
							By.id(newDealLocator.getString("select-ScheduledPaymentBudgetPurpose")))) {
						driver.findElement(By.id(newDealLocator.getString("select-ScheduledPaymentBudgetPurpose")))
								.findElement(By.tagName("input")).click();
						utility.selectFromDivMenu(driver, step,
								driver.findElement(
										By.id(newDealLocator.getString("select-ScheduledPaymentBudgetPurpose")))
										.findElement(By.tagName("ul")),
								paymentSubInstruction.getString("sub_instruction_budget_purpose"));
					}

					if (!scheduledInstruction.getBoolean("scheduled_payment_all_sub_instructions")) {
						driver.findElement(By.id(newDealLocator.getString("input-ScheduledPaymentDate")))
								.findElement(By.tagName("input")).sendKeys("sub_instruction_date");
					}

					switch (paymentSubInstruction.getString("sub_instruction_instrument").toLowerCase()) {
					case "bt":
						addBTSubInstruction(driver, step,
								driver.findElements(By.id(newDealLocator.getString("input-ScheduledPaymentValueDate")))
										.get(2),
					...
					driver.findElement(By.id(newDealLocator.getString("button-AddScheduledPaymentSubInstruction")))
							.click();
				}
			}
			...
			if (scheduledInstruction.getBoolean("scheduled_payment_enable_auto_retry")) {
				...
				driver.findElement(By.id(newDealLocator.getString("checkbox-ScheduledPaymentEnableAutoRetry")))
						.findElement(By.tagName("div")).click();

				driver.findElement(By.id(newDealLocator.getString("select-ScheduledPaymentRetryType")))
						.findElement(By.tagName("input")).click();
				utility.selectFromDivMenu(driver, step,
						driver.findElement(By.id(newDealLocator.getString("select-ScheduledPaymentRetryType")))
								.findElement(By.tagName("ul")),
						scheduledInstruction.getString("scheduled_payment_retry_type"));

				if (scheduledInstruction.getString("scheduled_payment_retry_type").toLowerCase().equals("same day")) {

					driver.findElement(By.id(newDealLocator.getString("input-ScheduledPaymentRetryHours")))
							.sendKeys(scheduledInstruction.getString("scheduled_payment_retry_hours"));
				}

				else if (scheduledInstruction.getString("scheduled_payment_retry_type").toLowerCase()
						.equals("custom")) {

					driver.findElement(By.id(newDealLocator.getString("input-ScheduledPaymentRetryDays")))
							.sendKeys(scheduledInstruction.getString("scheduled_payment_retry_days"));
				}
			}
			...
			if (scheduledInstruction.getBoolean("scheduled_payment_enable_notification_alerts")) {
				...
				driver.findElement(By.id(newDealLocator.getString("checkbox-ScheduledPaymentNotificationAlerts")))
						.findElement(By.tagName("div")).click();

				driver.findElement(By.id(newDealLocator.getString("select-ScheduledPaymentNotificationMode")))
						.findElement(By.tagName("input")).click();
				utility.selectFromDivMenu(driver, step,
						driver.findElement(By.id(newDealLocator.getString("select-ScheduledPaymentNotificationMode")))
								.findElement(By.tagName("ul")),
						scheduledInstruction.getString("scheduled_payment_notification_mode"));

				driver.findElement(By.id(newDealLocator.getString("button-ScheduledPaymentNotificationAddContact")))
						.click();

				List<WebElement> availableContacts = driver
						.findElements(By.xpath(newDealLocator.getString("label-ContactName")));
				List<String> actualContactList = new ArrayList<String>();
				for (WebElement contact : availableContacts) {
					actualContactList.add(contact.getText());
				}
				step.log(Status.INFO,
						"Contacts available at Scheduled Instruction - Expected: " + expectedContactList.toString()
								+ " Actual: " + actualContactList.toString() + " (See below image)");
				report.addStepInfoScreenshot(driver, step);
			...
			Select scheduledRetentionPurpose = new Select(
					driver.findElements(By.id(newDealLocator.getString("select-ScheduledRetentionPurpose"))).get(1));
			scheduledRetentionPurpose
					.selectByVisibleText(scheduledInstruction.getString("scheduled_retention_purpose"));

			driver.findElement(By.id(newDealLocator.getString("select-ScheduledRetentionExecute")))
					.findElement(By.tagName("input")).click();
			utility.selectFromDivMenu(driver, step,
					driver.findElement(By.id(newDealLocator.getString("select-ScheduledRetentionExecute"))).findElement(
							By.tagName("ul")),
					scheduledInstruction.getString("scheduled_retention_execute"));

			if (scheduledInstruction.getString("scheduled_retention_execute").toLowerCase()
					.equals("a few days prior")) {

				driver.findElement(By.id(newDealLocator.getString("input-ScheduledRetentionNoOfDays")))
						.sendKeys(scheduledInstruction.getString("scheduled_retention_noofdays"));
			}
			...
			Select scheduledSurplusPurpose = new Select(
					driver.findElements(By.id(newDealLocator.getString("select-ScheduledSurplusPurpose"))).get(1));
			...
			if (scheduledInstruction.has("scheduled_surplus_sub_instructions")) {
				...
				Iterator<Object> surplusSubInstructionsIterator = scheduledInstruction
						.getJSONArray("scheduled_surplus_sub_instructions").iterator();
				...
				while (surplusSubInstructionsIterator.hasNext()) {
					...
					driver.findElement(By.id(newDealLocator.getString("select-ScheduledSurplusInstrument")))
							.findElement(By.tagName("input")).click();
					utility.selectFromDivMenu(driver, step,
							driver.findElement(By.id(newDealLocator.getString("select-ScheduledSurplusInstrument")))
									.findElement(By.tagName("ul")),
							surplusSubInstruction.getString("sub_instruction_instrument"));

					driver.findElement(By.id(newDealLocator.getString("select-ScheduledSurplusBudget")))
							.findElement(By.tagName("input")).click();
					utility.selectFromDivMenu(driver, step,
							driver.findElement(By.id(newDealLocator.getString("select-ScheduledSurplusBudget")))
									.findElement(By.tagName("ul")),
							surplusSubInstruction.getString("sub_instruction_budget"));

					Thread.sleep(500);
					driver.findElement(By.id(newDealLocator.getString("select-ScheduledSurplusBudgetPurpose")))
							.findElement(By.tagName("input")).click();
					utility.selectFromDivMenu(driver, step,
							driver.findElement(By.id(newDealLocator.getString("select-ScheduledSurplusBudgetPurpose")))
									.findElement(By.tagName("ul")),
							surplusSubInstruction.getString("sub_instruction_budget_purpose"));
					...
					driver.findElement(By.id(newDealLocator.getString("button-AddScheduledSurplusSubInstruction")))
							.click();
				}
			}
	...
	private void addScheduledInstructions(WebDriver driver, ExtentTest step, JSONArray scheduledInstructions)
			throws Exception {
		...
		while (scheduledInstructionIterator.hasNext()) {
			...
			try {
				...
				case "payment-retention-surplus":
					Thread.sleep(500);
					driver.findElement(By.id(newDealLocator.getString("card-ScheduledPaymentRetentionSurplus")))
							.click();
				...
				step.log(Status.FAIL, "Error while adding Scheduled Instruction of type: "
						+ scheduledInstruction.getString("scheduled_type"));
				report.addStepFailScreenshot(driver, step);
				step.log(Status.FAIL, e.getMessage());
				throw new Exception("Error while adding Scheduled Instruction of type: "
						+ scheduledInstruction.getString("scheduled_type"));
			}
			driver.findElement(By.xpath(newDealLocator.getString("button-ScheduledBack"))).click();
		}
	...
	private void addLinkedPayment(WebDriver driver, ExtentTest step, JSONObject linkedInstruction) throws Exception {
		try {
			utility.addToExpectedDetails(expectedAccountDetails,
					linkedInstruction.getString("linked_payment_source_account").split("-")[0].toLowerCase().trim(),
					"name:" + linkedInstruction.getString("linked_payment_name").toLowerCase().trim() + ";" + "purpose:"
							+ linkedInstruction.getString("linked_payment_purpose").toLowerCase().trim() + ";"
							+ "type:payment");

			driver.findElement(By.id(newDealLocator.getString("input-ScheduledPaymentName"))).clear();
			driver.findElement(By.id(newDealLocator.getString("input-ScheduledPaymentName")))
					.sendKeys(linkedInstruction.getString("linked_payment_name"));

			Select linkedPaymentPurpose = new Select(
					driver.findElements(By.id(newDealLocator.getString("select-ScheduledPaymentPurpose"))).get(1));
			...
			List<String> actualSourceAccounts = utility.extractOptionsFromSelect(driver, step,
					driver.findElement(By.id(newDealLocator.getString("select-ScheduledPaymentSourceAccount")))
							.findElement(By.tagName("ul")));
			step.log(Status.INFO, "Source Accounts available at Linked Instruction - Expected: "
					+ expectedSourceAccounts.toString() + " Actual: " + actualSourceAccounts.toString());
			report.addStepInfoScreenshot(driver, step);
			utility.selectFromDivMenu(driver, step,
					driver.findElement(By.id(newDealLocator.getString("select-ScheduledPaymentSourceAccount")))
							.findElement(By.tagName("ul")),
					linkedInstruction.getString("linked_payment_source_account"));

			Select linkedPaymentBalanceConsideration = new Select(
					driver.findElements(By.id(newDealLocator.getString("select-ScheduledPaymentBalanceConsideration")))
							.get(1));
			linkedPaymentBalanceConsideration
					.selectByVisibleText(linkedInstruction.getString("linked_payment_balance_consideration"));
			...
			if (linkedInstruction.getBoolean("linked_payment_repeating")) {
				...
				List<WebElement> linkedInstructionDatePicker = driver
						.findElements(By.id(newDealLocator.getString("input-ScheduledPaymentDatePicker")));
				...
				if (linkedInstruction.getBoolean("linked_payment_all_sub_instructions")) {
					...
					Select linkedInstructionFrequency = new Select(driver
							.findElements(By.id(newDealLocator.getString("select-ScheduledPaymentFrequency"))).get(1));
					linkedInstructionFrequency
							.selectByVisibleText(linkedInstruction.getString("linked_payment_frequency"));

					driver.findElement(By.id(newDealLocator.getString("input-ScheduledPaymentRepeatEvery"))).clear();
					driver.findElement(By.id(newDealLocator.getString("input-ScheduledPaymentRepeatEvery")))
							.sendKeys(linkedInstruction.getString("linked_payment_repeat_every"));
					...
					case "monthly":
						...
						Select linkedInstructionFrequencySubtype = new Select(
								driver.findElements(By.id(newDealLocator.getString("select-ScheduledPaymentSubType")))
										.get(1));
						linkedInstructionFrequencySubtype
								.selectByVisibleText(linkedInstruction.getString("linked_payment_sub_type"));

						if (linkedInstruction.getString("linked_payment_sub_type").toLowerCase()
								.equals("day of month")) {
							...
							utility.selectFromDivMenu(driver, step,
									driver.findElement(By.id(newDealLocator.getString("select-ScheduledPaymentDays")))
											.findElement(By.tagName("ul")),
									linkedInstruction.getJSONArray("linked_payment_days"));
						} else if (linkedInstruction.getString("linked_payment_sub_type").toLowerCase()
								.equals("weekday(s)")) {

							Select linkedInstructionFrequencyWeek = new Select(
									driver.findElements(By.id(newDealLocator.getString("select-ScheduledPaymentWeek")))
											.get(1));
							linkedInstructionFrequencyWeek
									.selectByVisibleText(linkedInstruction.getString("linked_payment_week"));

							selectDayOfWeek(driver, step,
									linkedInstruction.getJSONArray("linked_payment_days_of_week"));
						}
						break;

					default:
						step.log(Status.FAIL, "Invalid payment frequency: "
								+ linkedInstruction.getString("linked_payment_frequency"));
						throw new Exception("Invalid payment frequency: "
								+ linkedInstruction.getString("linked_payment_frequency"));
					}
				} else {
				...
				driver.findElement(By.id(newDealLocator.getString("input-ScheduledPaymentDatePicker")))
						.sendKeys(linkedInstruction.getString("linked_payment_execution_date"));
			}

			Select linkedInstructionScheduledAt = new Select(
					driver.findElements(By.id(newDealLocator.getString("select-ScheduledPaymentScheduledAt"))).get(1));
			linkedInstructionScheduledAt
					.selectByVisibleText(linkedInstruction.getString("linked_payment_scheduled_at"));

			Select linkedInstructionHolidayAction = new Select(driver
					.findElements(By.id(newDealLocator.getString("select-ScheduledPaymentHolidayAction"))).get(1));
			linkedInstructionHolidayAction
					.selectByVisibleText(linkedInstruction.getString("linked_payment_holiday_action"));

			driver.findElement(By.id(newDealLocator.getString("button-ScheduledPaymentScheduleNext"))).click();

			if (linkedInstruction.has("linked_payment_sub_instructions")) {

				Iterator<Object> paymentSubInstructionsIterator = linkedInstruction
						.getJSONArray("linked_payment_sub_instructions").iterator();
				...
				while (paymentSubInstructionsIterator.hasNext()) {
					...
					utility.selectFromDivMenu(driver, step,
							driver.findElement(By.id(newDealLocator.getString("select-ScheduledPaymentBudget")))
									.findElement(By.tagName("ul")),
							paymentSubInstruction.getString("sub_instruction_budget"));
			...
			if (linkedInstruction.getBoolean("linked_payment_enable_auto_retry")) {
				...
				utility.selectFromDivMenu(driver, step,
						driver.findElement(By.id(newDealLocator.getString("select-ScheduledPaymentRetryType")))
								.findElement(By.tagName("ul")),
						linkedInstruction.getString("linked_payment_retry_type"));

				if (linkedInstruction.getString("linked_payment_retry_type").toLowerCase().equals("same day")) {

					driver.findElement(By.id(newDealLocator.getString("input-ScheduledPaymentRetryHours")))
							.sendKeys(linkedInstruction.getString("linked_payment_retry_hours"));
				}

				else if (linkedInstruction.getString("linked_payment_retry_type").toLowerCase().equals("custom")) {

					driver.findElement(By.id(newDealLocator.getString("input-ScheduledPaymentRetryDays")))
							.sendKeys(linkedInstruction.getString("linked_payment_retry_days"));
				}
			}
			...
			if (linkedInstruction.getBoolean("linked_payment_enable_notification_alerts")) {
				...
				utility.selectFromDivMenu(driver, step,
						driver.findElement(By.id(newDealLocator.getString("select-ScheduledPaymentNotificationMode")))
								.findElement(By.tagName("ul")),
						linkedInstruction.getString("linked_payment_notification_mode"));
				...
				step.log(Status.INFO,
						"Contacts available at Linked Instruction - Expected: " + expectedContactList.toString()
								+ " Actual: " + actualContactList.toString() + " (See below image)");
				report.addStepInfoScreenshot(driver, step);
	...
	private void addLinkedInstructions(WebDriver driver, ExtentTest step, JSONArray linkedInstructions)
			throws Exception {
		...
		while (linkedInstructionIterator.hasNext()) {
			...
			try {
				...
				step.log(Status.FAIL,
						"Error while adding Linked Instruction of type: " + linkedInstruction.getString("linked_type"));
				report.addStepFailScreenshot(driver, step);
				step.log(Status.FAIL, e.getMessage());
				throw new Exception(
						"Error while adding Linked Instruction of type: " + linkedInstruction.getString("linked_type"));
	...
	private void fillDocument(WebDriver driver, ExtentTest step, JSONObject document) throws Exception {
		...
		try {
			...
			driver.findElement(By.id(newDealLocator.getString("select-DocumentType"))).findElement(By.tagName("input"))
					.click();
			utility.selectFromDivMenu(driver, step, driver
					.findElement(By.id(newDealLocator.getString("select-DocumentType"))).findElement(By.tagName("ul")),
					document.getString("document_type"));

			Select documentNature = new Select(
					driver.findElement(By.xpath(newDealLocator.getString("select-DocumentNature"))));
			documentNature.selectByVisibleText(document.getString("document_nature"));

			if (document.has("document_date")) {
				driver.findElement(By.id(newDealLocator.getString("input-DocumentDatePicker")))
						.sendKeys(document.getString("document_date"));
			}
	...
	private void addDocuments(WebDriver driver, ExtentTest step, String dealName, JSONArray documents)
			throws Exception {
		...
		while (documentIterator.hasNext()) {
			...
			try {
				utility.addToExpectedDetails(expectedDealDetails, dealName.toLowerCase().trim(),
						"document level:" + document.getString("document_level").toLowerCase().trim()
								+ ";document type:" + document.getString("document_type").toLowerCase().trim());
				Thread.sleep(1000);
				...
				case "mandatory":
					...
					if (document.getBoolean("document_deferral")) {
						...
						driver.findElement(By.id(newDealLocator.getString("input-DocumentDatePicker")))
								.sendKeys(document.getString("document_deferral_due_date"));
					...
					driver.findElement(By.id(newDealLocator.getString("input-DocumentUpload")))
							.sendKeys(documentFile.getAbsolutePath());
					break;
	...
	private void addNotifications(WebDriver driver, ExtentTest step, String dealName, JSONArray notifications)
			throws Exception {
		...
		while (notificationIterator.hasNext()) {
			List<String> notificationList = utility.extractColumnsFromTable(driver, step,
					By.id(newDealLocator.getString("table-NotificationsRows")), 0);

			JSONObject notification = (JSONObject) notificationIterator.next();
			try {
				utility.addToExpectedDetails(expectedDealDetails, dealName.toLowerCase().trim(),
						"notification:" + notification.getString("notification_type").toLowerCase().trim());

				WebElement notificationElement = driver
						.findElements(By.id(newDealLocator.getString("table-NotificationsRows")))
						.get(notificationList.indexOf(notification.getString("notification_type")));
				...
				driver.findElement(By.id(newDealLocator.getString("menu-NotificationsPopupMenu")))
						.findElement(By.tagName("div")).click();

				driver.findElement(By.id(newDealLocator.getString("checkbox-NotificationsEnable")))
						.findElement(By.tagName("label")).click();

				Thread.sleep(500);
				driver.findElement(By.id(newDealLocator.getString("select-NotificationsMode")))
						.findElement(By.tagName("input")).click();
				utility.selectFromDivMenu(driver, step,
						driver.findElement(By.id(newDealLocator.getString("select-NotificationsMode")))
								.findElement(By.tagName("ul")),
						notification.getString("notification_mode"));
				...
				step.log(Status.INFO,
						"Contacts available at Notifications - Expected: " + expectedContactList.toString()
								+ " Actual: " + actualContactList.toString() + " (See below image)");
				report.addStepInfoScreenshot(driver, step);
				...
				step.log(Status.INFO,
						"Notification: " + notification.getString("notification_type") + " added to deal");
			} catch (Exception e) {
				step.log(Status.FAIL,
						"Error while enabling Notification: " + notification.getString("notification_type"));
				report.addStepFailScreenshot(driver, step);
				step.log(Status.FAIL, e.getMessage());
				throw new Exception(
						"Error while enabling Notification: " + notification.getString("notification_type"));
	...
	private void addExecutionPolicies(WebDriver driver, ExtentTest step, String dealName, JSONObject executionPolicies)
			throws Exception {
		try {
			driver.findElement(By.id(newDealLocator.getString("tab-ExecutionPoliciesWizard"))).click();
			utility.waitForProgressBarToLoad(driver);

			Select executionPolicyDependencies = new Select(
					driver.findElement(By.id(newDealLocator.getString("select-ExecutionPoliciesDependencies")))
							.findElement(By.tagName("select")));
			executionPolicyDependencies
					.selectByVisibleText(executionPolicies.getString("execution_policy_dependencies"));

			Select executionPolicyVerifyBalances = new Select(
					driver.findElement(By.id(newDealLocator.getString("select-ExecutionPoliciesVerifyBalances")))
							.findElement(By.tagName("select")));
			executionPolicyVerifyBalances
					.selectByVisibleText(executionPolicies.getString("execution_policy_verify_balances"));

			Select executionPolicyReattempt = new Select(
					driver.findElement(By.id(newDealLocator.getString("select-ExecutionPoliciesReattempt")))
							.findElement(By.tagName("select")));
			executionPolicyReattempt.selectByVisibleText(executionPolicies.getString("execution_policy_reattempt"));

			Select executionPolicyHolidayAction = new Select(
					driver.findElement(By.id(newDealLocator.getString("select-ExecutionPoliciesHolidayAction")))
							.findElement(By.tagName("select")));
			executionPolicyHolidayAction
					.selectByVisibleText(executionPolicies.getString("execution_policy_holiday_action"));

			Select executionPolicyInactiveBehaviour = new Select(
					driver.findElement(By.id(newDealLocator.getString("select-ExecutionPoliciesInactiveBehaviour")))
							.findElement(By.tagName("select")));
			executionPolicyInactiveBehaviour
					.selectByVisibleText(executionPolicies.getString("execution_policy_inactive_behaviour"));

			if (executionPolicies.getString("execution_policy_verify_balances").equals("At an Instruction")) {
				Select executionPolicyBalanceConsideration = new Select(driver
						.findElement(By.id(newDealLocator.getString("select-ExecutionPoliciesBalanceConsideration")))
						.findElement(By.tagName("select")));
				executionPolicyBalanceConsideration
						.selectByVisibleText(executionPolicies.getString("execution_policy_balance_consideration"));
			}

			List<WebElement> executionPolicyReattemptInterval = driver
					.findElements(By.xpath(newDealLocator.getString("button-ExecutionPoliciesReattemptInterval")));
			...
			default:
				step.log(Status.FAIL, "Invalid Reattempt Interval: "
						+ executionPolicies.getString("execution_policy_reattempt_interval"));
				report.addStepFailScreenshot(driver, step);
				throw new Exception("Invalid Reattempt Interval: "
						+ executionPolicies.getString("execution_policy_reattempt_interval"));
			}
	...
	private List<String> extractTextFromBudgetGroupCards(WebDriver driver, ExtentTest step) throws Exception {
		...
		try {
			List<WebElement> budgetGroupCardElements = driver
					.findElements(By.id(newDealLocator.getString("card-BudgetDetails")));
	...
	private List<String> extractTextFromScheduledInstructionCards(WebDriver driver, ExtentTest step) throws Exception {
		...
		try {
			List<WebElement> scheduledInstructionCardElements = driver
					.findElement(By.id(newDealLocator.getString("card-ScheduledInstructionDetails")))
			...
			for (WebElement scheduledInstructionCard : scheduledInstructionCardElements) {
				...
				scheduledInstructionList
						.add(scheduledInstructionName + scheduledInstructionPurpose + scheduledInstructionType);
			}
		} catch (Exception e) {
	...
	private List<String> extractTextFromLinkedInstructionCards(WebDriver driver, ExtentTest step) throws Exception {
		...
		try {
			List<WebElement> linkedInstructionCardElements = driver
					.findElement(By.id(newDealLocator.getString("card-LinkedInstructionDetails")))
					.findElements(By.xpath("*"));
	...
	private List<String> extractTextFromEntitlementCards(WebDriver driver, ExtentTest step) throws Exception {
		...
		try {
			List<String> entitlementRecords = utility.extractColumnsFromTable(driver, step,
					By.id(newDealLocator.getString("table-EntitlementList")), 2);
	...
	private void assertAccountSection(WebDriver driver, ExtentTest step, JSONObject deal) throws Exception {
		utility.waitForElementToBeDisplayed(driver, By.xpath(newDealLocator.getString("button-SummaryAccountNumber")),
				2, 120);
		...
		try {
			while (index < noOfEntries) {
				String accountNumber = driver
						.findElements(By.xpath(newDealLocator.getString("button-SummaryAccountNumber"))).get(index)
						.getText().toLowerCase().trim();

				if (deal.has("budget_groups")
						&& !driver.findElements(By.xpath(newDealLocator.getString("button-SummaryAccountBudgetGroups")))
								.get(index).getText().equals("0")) {
					driver.findElements(By.xpath(newDealLocator.getString("button-SummaryAccountBudgetGroups")))
							.get(index).findElement(By.tagName("div")).click();
				...
				if (deal.has("scheduled_instructions") && !driver
						.findElements(By.xpath(newDealLocator.getString("button-SummaryAccountScheduledInstructions")))
						.get(index).getText().equals("0")) {
					driver.findElements(
							By.xpath(newDealLocator.getString("button-SummaryAccountScheduledInstructions"))).get(index)
							.findElement(By.tagName("div")).click();
					...
					step.log(Status.INFO,
							"Scheduled Instructions added to account " + accountNumber + " (See below image)");
					report.addStepInfoScreenshot(driver, step);
					driver.findElement(By.xpath(newDealLocator.getString("button-SummaryClose"))).click();
				}

				if (deal.has("linked_instructions") && !driver
						.findElements(By.xpath(newDealLocator.getString("button-SummaryAccountLinkedInstructions")))
						.get(index).getText().equals("0")) {
					driver.findElements(By.xpath(newDealLocator.getString("button-SummaryAccountLinkedInstructions")))
							.get(index).findElement(By.tagName("div")).click();
					...
					step.log(Status.INFO,
							"Linked Instructions added to account " + accountNumber + " (See below image)");
					report.addStepInfoScreenshot(driver, step);
					driver.findElement(By.xpath(newDealLocator.getString("button-SummaryClose"))).click();
				}

				if (deal.has("entitlements")
						&& !driver.findElements(By.xpath(newDealLocator.getString("button-SummaryAccountEntitlements")))
								.get(index).getText().equals("0")) {
					driver.findElements(By.xpath(newDealLocator.getString("button-SummaryAccountEntitlements")))
							.get(index).findElement(By.tagName("div")).click();
				...
				if (!expectedAccountDetails.get(accountNumber).isEmpty()) {
					...
					step.log(Status.FAIL,
							"Components not added: " + expectedAccountDetails.get(accountNumber).toString());
				} else {
	...
	private void assertPartySection(WebDriver driver, ExtentTest step, JSONObject deal) throws Exception {
		utility.waitForElementToBeDisplayed(driver, By.xpath(newDealLocator.getString("button-SummaryPartyName")), 2,
				120);
		int noOfEntries = driver.findElements(By.xpath(newDealLocator.getString("button-SummaryPartyName"))).size();
		int index = 1;

		utility.scrollIntoView(driver,
				driver.findElements(By.xpath(newDealLocator.getString("button-SummaryPartyName"))).get(0));
		step.log(Status.INFO, "Parties added to  deal " + deal.getString("name") + " (See below image)");
		report.addStepInfoScreenshot(driver, step);

		try {
			while (index < noOfEntries) {
				String partyName = driver.findElements(By.xpath(newDealLocator.getString("button-SummaryPartyName")))
						.get(index).getText().toLowerCase().trim();

				utility.scrollIntoView(driver,
						driver.findElements(By.xpath(newDealLocator.getString("button-SummaryPartyName"))).get(index));
				Thread.sleep(500);

				String partyParticipantID = "participant:"
						+ driver.findElements(By.xpath(newDealLocator.getString("button-SummaryPartyParticipantID")))
								.get(index).getText().toLowerCase().trim();
				...
				String partyResponsibility = "responsibility:"
						+ driver.findElements(By.xpath(newDealLocator.getString("button-SummaryPartyResponsibility")))
								.get(index).getText().toLowerCase().trim();
				...
				if (expectedPartyDetails.get(partyName).toString().contains("debit:")) {
					driver.findElements(By.xpath(newDealLocator.getString("button-SummaryPartyDebits"))).get(index)
							.findElement(By.tagName("div")).click();

					List<String> partyDebitList = utility.extractColumnsFromTable(driver, step,
							By.id(newDealLocator.getString("table-NotificationsRows")), 0);
				...
				if (expectedPartyDetails.get(partyName).toString().contains("contact:")) {
					driver.findElements(By.xpath(newDealLocator.getString("button-SummaryPartyContacts"))).get(index)
							.findElement(By.tagName("div")).click();

					List<String> partyContactList = utility.extractColumnsFromTable(driver, step,
							By.id(newDealLocator.getString("table-NotificationsRows")), 1);
				...
				if (expectedPartyDetails.get(partyName).toString().contains("account:")) {
					driver.findElements(By.xpath(newDealLocator.getString("button-SummaryPartyAccounts"))).get(index)
							.findElement(By.tagName("div")).click();
					List<String> partyAccountList = utility.extractColumnsFromTable(driver, step,
							By.id(newDealLocator.getString("table-NotificationsRows")), 1);
				...
				if (expectedPartyDetails.get(partyName).toString().contains("document:")) {
					utility.scrollIntoView(driver,
							driver.findElements(By.xpath(newDealLocator.getString("button-SummaryPartyDocuments")))
									.get(index));
					Thread.sleep(500);
					driver.findElements(By.xpath(newDealLocator.getString("button-SummaryPartyDocuments"))).get(index)
							.findElement(By.tagName("div")).click();
					List<String> partyDocumentList = utility.extractColumnsFromTable(driver, step,
							By.id(newDealLocator.getString("table-NotificationsRows")), 0);
	...
	private String extractTextFromFeesCards(WebDriver driver, ExtentTest step) throws Exception {
		...
		String feeType = "type:";
		try {
			List<WebElement> feeCardElements = driver.findElement(By.id(newDealLocator.getString("card-FeeDetails")))
					.findElements(By.xpath("*"));
	...
	private void assertFeeSection(WebDriver driver, ExtentTest step, JSONObject deal) throws Exception {
		...
		try {
			List<WebElement> feeList = driver.findElement(By.id(newDealLocator.getString("grid-SummaryFee")))
	...
	private boolean assertDealDetails(WebDriver driver, ExtentTest step, JSONObject deal) throws Exception {
		...
		try {
			...
			step.log(Status.INFO, "Deal Starts On - Expected: " + deal.getString("starts_on") + " Actual: "
					+ dealStartsOn.getText() + " (See below image)");
			report.addStepInfoScreenshot(driver, step);
			...
			if (deal.has("ends_on")) {
				...
				step.log(Status.INFO, "Deal Ends On - Expected: " + deal.getString("ends_on") + " Actual: "
						+ dealEndsOn.getText() + " (See below image)");
				report.addStepInfoScreenshot(driver, step);
			...
			Map<String, String> country = Map.of("IND", "India", "MY", "Malaysia", "GB", "United Kingdom", "SG",
					"Singapore", "US", "America");
			step.log(Status.INFO, "Deal Country - Expected: " + deal.getString("country") + " Actual: "
					+ country.get(dealCountry.getText()) + " (See below image)");
			report.addStepInfoScreenshot(driver, step);
			...
			step.log(Status.INFO, "Deal Timezone - Expected: " + deal.getString("timezone") + " Actual: "
					+ dealTimezone.getText() + " (See below image)");
			report.addStepInfoScreenshot(driver, step);
			if (!deal.getString("timezone").contains(dealTimezone.getText())) {
				result = false;
			}

			if (deal.has("entitlements") && !driver
					.findElement(By.id(newDealLocator.getString("button-SummaryEntitlements"))).getText().equals("0")) {
				utility.scrollIntoView(driver,
						driver.findElement(By.id(newDealLocator.getString("button-SummaryEntitlements"))));
				...
				step.log(Status.INFO, "Entitlements added to deal " + deal.getString("name").toLowerCase().trim()
						+ " (See below image)");
				report.addStepInfoScreenshot(driver, step);
				driver.findElement(By.xpath(newDealLocator.getString("button-SummaryClose"))).click();
			}
			...
			utility.scrollIntoView(driver,
					driver.findElement(By.id(newDealLocator.getString("button-SummaryProcessingUnits"))));
			driver.findElement(By.id(newDealLocator.getString("button-SummaryProcessingUnits"))).click();
			Thread.sleep(1000);
			String[] dealProcessingUnits = driver
					.findElement(By.id(newDealLocator.getString("card-SummaryProcessingUnits"))).getText().split("\n");
			expectedDealDetails.get(deal.getString("name").toLowerCase().trim())
					.remove("processing units:" + Arrays.toString(dealProcessingUnits).toLowerCase().trim());

			step.log(Status.INFO, "Processing units added to deal " + deal.getString("name").toLowerCase().trim()
					+ " (See below image)");
			report.addStepInfoScreenshot(driver, step);
			driver.findElement(By.xpath(newDealLocator.getString("button-SummaryClose"))).click();
			...
			if (deal.has("notifications")) {
				utility.scrollIntoView(driver,
						driver.findElement(By.id(newDealLocator.getString("button-SummaryNotifications"))));
				driver.findElement(By.id(newDealLocator.getString("button-SummaryNotifications"))).click();
				Thread.sleep(2000);
				List<String> notificationTypes = utility.extractColumnsFromTable(driver, step,
						By.id(newDealLocator.getString("table-NotificationsRows")), 0);
				for (String notification : notificationTypes) {

					expectedDealDetails.get(deal.getString("name").toLowerCase().trim())
							.remove("notification:" + notification.toLowerCase().trim());
				}

				step.log(Status.INFO, "Notifications added to deal " + deal.getString("name").toLowerCase().trim()
						+ " (See below image)");
				report.addStepInfoScreenshot(driver, step);
				driver.findElement(By.xpath(newDealLocator.getString("button-SummaryClose"))).click();
			}
			...
			if (deal.has("contacts")) {
				utility.scrollIntoView(driver,
						driver.findElement(By.id(newDealLocator.getString("button-SummaryContacts"))));
				driver.findElement(By.id(newDealLocator.getString("button-SummaryContacts"))).click();
				List<String> contactList = utility.extractColumnsFromTable(driver, step,
						By.id(newDealLocator.getString("table-NotificationsRows")), 0);
				for (String contact : contactList) {
					expectedDealDetails.get(deal.getString("name").toLowerCase().trim())
							.remove("contact:" + contact.toLowerCase().trim());
				}

				step.log(Status.INFO,
						"Contacts added to deal " + deal.getString("name").toLowerCase().trim() + " (See below image)");
				report.addStepInfoScreenshot(driver, step);
				driver.findElement(By.xpath(newDealLocator.getString("button-SummaryClose"))).click();
			}
			...
			if (deal.has("documents")) {
				utility.scrollIntoView(driver,
						driver.findElement(By.id(newDealLocator.getString("button-SummaryDocuments"))));
				driver.findElement(By.id(newDealLocator.getString("button-SummaryDocuments"))).click();
				Thread.sleep(2000);
				List<String> documentList = utility.extractColumnsFromTable(driver, step,
						By.xpath(newDealLocator.getString("table-SummaryDocuments")), 2);
				for (String document : documentList) {

					expectedDealDetails.get(deal.getString("name").toLowerCase().trim())
							.remove("document level:mandatory;document type:" + document.toLowerCase().trim());
				}

				step.log(Status.INFO, "Mandatory documents added to deal " + deal.getString("name").toLowerCase().trim()
						+ " (See below image)");
				report.addStepInfoScreenshot(driver, step);
				driver.findElement(By.xpath(newDealLocator.getString("button-SummaryClose"))).click();
			}
			...
			try {
				if (deal.has("deal_response_messages")) {
					...
					if (!actualResponseMessages.isEmpty()) {
						step.log(Status.PASS,
								"Additional messages: " + actualResponseMessages.toString() + " displayed");
		...
		while (dealIterator.hasNext()) {
			...
			try {
				...
				Select productSelect = new Select(
						driver.findElements(By.id(newDealLocator.getString("select-Product"))).get(1));
				...
				Select businessSegementSelect = new Select(
						driver.findElements(By.id(newDealLocator.getString("select-BusinessSegment"))).get(1));
				businessSegementSelect.selectByVisibleText(deal.getString("business_segment"));

				driver.findElement(By.id(newDealLocator.getString("select-ProcessingUnit")))
						.findElement(By.tagName("input")).click();
				Thread.sleep(1000);
				driver.findElement(By.id(newDealLocator.getString("select-ProcessingUnit")))
						.findElements(By.tagName("ul")).get(1).findElements(By.tagName("span")).get(1).click();
				utility.selectFromDivMenu(driver, step,
						driver.findElement(By.xpath(newDealLocator.getString("select-ChildProcessingUnit"))),
						deal.getJSONArray("processing_units"));
				List<Object> updatedProcessingUnits = deal.getJSONArray("processing_units").toList();

				utility.addToExpectedDetails(expectedDealDetails, deal.getString("name").toLowerCase().trim(),
						"processing units:" + updatedProcessingUnits.toString().toLowerCase().trim());

				Select countrySelect = new Select(
						driver.findElements(By.id(newDealLocator.getString("select-Country"))).get(1));
				countrySelect.selectByVisibleText(deal.getString("country"));

				Select timezoneSelect = new Select(
						driver.findElements(By.id(newDealLocator.getString("select-Timezone"))).get(1));
				...
				if (deal.has("execution_policies")) {
					addExecutionPolicies(driver, step, deal.getString("name"),
							deal.getJSONObject("execution_policies"));
				}

				Thread.sleep(2000);
				driver.findElement(By.id(newDealLocator.getString("tab-SummaryWizard"))).click();

				dealDetails.put(deal.getString("name").toLowerCase().trim(),
						driver.findElement(By.xpath(newDealLocator.getString("label-SummaryHeading")))
								.findElements(By.tagName("span")).get(2).getText());
	...
	public void createDeal(WebDriver driver, ExtentTest step, JSONObject deal) throws Exception {
		try {
			...
			driver.findElement(By.id(newDealLocator.getString("select-ProcessingUnit")))
					.findElement(By.tagName("input")).click();
			Thread.sleep(1000);
			driver.findElement(By.id(newDealLocator.getString("select-ProcessingUnit"))).findElements(By.tagName("ul"))
					.get(1).findElements(By.tagName("span")).get(1).click();
			utility.selectFromDivMenu(driver, step,
					driver.findElement(By.xpath(newDealLocator.getString("select-ChildProcessingUnit"))),
					deal.getJSONArray("processing_units"));
			List<Object> updatedProcessingUnits = deal.getJSONArray("processing_units").toList();
	...
	public void createNewDealsDraft(WebDriver driver, ExtentTest step) throws Exception {
		deals = testData.getJSONArray("deals");
		Iterator<Object> dealIterator = deals.iterator();
		while (dealIterator.hasNext()) {
			JSONObject deal = (JSONObject) dealIterator.next();
			try {
				...
				if (driver.findElements(By.xpath(newDealLocator.getString("checkbox-LinkPartySelect")))
					...
					expectedPartyDetails.remove(deal.getString("name").toLowerCase().trim());
					driver.findElement(By.xpath(newDealLocator.getString("button-SummaryClose"))).click();
